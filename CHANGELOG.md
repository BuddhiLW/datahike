...
- **Configurable index-node reference strength** — `datahike.index.persistent-set/map->settings` hardcoded a `nil` `RefType` (which resolves to `SOFT`) while a comment claimed "weak ref default." The persistent-sorted-set `Settings` now accept `{:ref-type :weak | :soft | :strong}`: `:weak` releases storage-backed index nodes at the next GC (lazy re-read, lowest heap residency), `:soft` (default, unchanged behavior) releases only under heap pressure, `:strong` pins them. Lets memory-constrained deployments trade re-fetch I/O for a smaller resident index without patching the library. *Experimental.* (0.8, [#TODO])
